...
QBCore.Functions.CreateCallback("qb-garage:server:checkVehiclePD", function(source, cb, plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    exports.oxmysql:execute('SELECT * FROM player_vehicles WHERE plate = ? AND job = ?',{plate, "police"}, function(result)
        if result[1] then
            cb(true)
        else
            cb(false)
...
QBCore.Functions.CreateCallback("qb-garage:server:GetUserVehiclesPD", function(source, cb, garage)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    exports.oxmysql:execute('SELECT * FROM player_vehicles WHERE job = ? AND garage = ?', {"police", garage}, function(result)
        if result[1] then
            cb(result)
        else
            cb(nil)
