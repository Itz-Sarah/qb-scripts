...
Citizen.CreateThread(function()
    mainblip = AddBlipForCoord(-438.4671, 6147.586, 31.47821)
    ...
    AddTextComponentString("Royal Mail")
...
local royalmail = {
    {x = -438.4671,  y = 6147.586,  z = 31.378} 
}
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        for k in pairs(royalmail) do
            local plyCoords = GetEntityCoords(PlayerPedId(), false)
            local dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, royalmail[k].x, royalmail[k].y, royalmail[k].z)
   
            if dist <= 5 then
                --DrawText3Ds(royalmail[k].x, royalmail[k].y, royalmail[k].z+1, ('Press [~g~E~w~] to start delivering'))
            end
        end
    end
end)
...
function delivered()
    TriggerServerEvent('royalmail:success')
        if visits == 2 then --change 3 to however many runs you want a person to be able to make before having to return to the depot
            RemoveBlip(deliveryblip)
            visits = 1
...
RegisterNetEvent('qb-royalmail:client:rmail')
AddEventHandler('qb-royalmail:client:rmail', function()
    startDeliver()
end)
